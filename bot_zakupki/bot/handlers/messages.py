...
CMD_START_MSG = (
    ...
    "\n\n"
    "Каждый будний день, в 8 утра по Москве, мы будем присылать список "
...
CMD_HELP_MSG = (
    ...
    f"/{commands.SHOW_ALL_MY_QUERIES} - отображение всех запросов\n"
    f"/{commands.CHANGE_QUERY} - при помощи этой команды можно изменить "
    "какой-нибудь запрос\n"
    f"/{commands.UPDATE_SUBSCRIPTION} - управление подпиской\n"
...
def subscription_message_formation(user_id: str):
    ...
    if subscription_last_day is None or subscription_last_day < now:
        ...
        subscription_msg = (
            ...
            "\n"
            f"Всего существует 6 вариантов подписки.\n"
            f"Они различаются количеством дней и количеством запросов. "
            f"Каждый запрос состоит из 3-х параметров: "
            f"предмет закупки, интервал цен и регион закупки.\n"
            "\n"
            f"Например, подписка на 90 дней и на 5 запросов: "
            f"в течение 90 дней, каждый рабочий день, в 8 утра, "
            f"бот будет присылать подходящие закупки по 5-ти запросам. "
            f"По каждому запросу будет сформирована табличка с подходящими "
            f"закупками, размещенными в предыдущий день."
        )
